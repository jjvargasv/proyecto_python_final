...
// Página para mostrar el historial de órdenes del usuario
export default function Ordenes() {
  ...
  // Estado para almacenar los productos de la orden seleccionada
  const [productosOrden, setProductosOrden] = useState([]);
  // Estado para filtrar las órdenes
  ...
  // Filtrado de órdenes según el estado
  ...
  // Renderizado de la página de órdenes
  return (
            ...
            <table className="w-full text-white mb-4">
              ...
              <tbody>
                {ordenesFiltradas.map((orden, i) => (
                  <tr key={orden.id} className="border-b border-gray-800 hover:bg-purple-900/30">
                    <td className="py-2">{orden.id}</td>
                    <td className="py-2">{new Date(orden.created_at).toLocaleString()}</td>
                    <td className="py-2">${orden.total || orden.total_price || "-"}</td>
                    <td className="py-2">{orden.estado || orden.status || "-"}</td>
                    <td className="py-2">
                      <button className="text-purple-300 underline" onClick={() => handleVerDetalleOrden(orden)}>Ver detalle</button>
                    </td>
                  </tr>
                ))}
              </tbody>
